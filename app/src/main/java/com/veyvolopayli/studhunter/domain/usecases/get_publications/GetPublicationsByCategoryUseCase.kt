...
class GetPublicationsByCategoryUseCase @Inject constructor(
    private val repository: PublicationRepository
) {
    operator fun invoke(category: String): Flow<Resource<List<Publication>>> = flow {
        try {
            emit(Resource.Loading<List<Publication>>())
            val publications =
                repository.getPublicationsByCategory(category = category).map { it.toPublication() }
