...
class PublicationRepositoryImpl(private val api: StudHunterApi): PublicationRepository {
    ...
    override suspend fun fetchPublications(): List<Publication> {
        return api.fetchPublications().map { it.toPublication() }
    }

    override suspend fun fetchPublication(token: String, id: String): DetailedPublication {
        return api.fetchPublication(token, id).toDetailedPublication()
    }
    ...
    override suspend fun searchPublications(query: String): List<Publication> {
        return api.searchPublications(query).map { it.toPublication() }
    }
