...
//                url("ws://5.181.255.253/chat?chatID=$chatID")
    ...
    override suspend fun getMessagesByChatId(token: String, chatId: String) =
        api.getMessagesByChatId(token = token, chatId = chatId).map { it.toMessage() }

    override suspend fun getMessagesByPublicationId(token: String, pubId: String) =
        api.getMessagesByPublicationId(token = token, pubId = pubId).map { it.toMessage() }
